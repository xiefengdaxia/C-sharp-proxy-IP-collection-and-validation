...
namespace 代理ip抓取forWPF
{
    class helper
    {
            ...
            public static 代理ip抓取forWPF.MainWindow.queueresult DownLoadHtml(string url, string proxyip, 代理ip抓取forWPF.MainWindow.queueresult q, string localhostIpAddress, int timeout = 10, bool enableProxy = true)
            {
                var result = new 代理ip抓取forWPF.MainWindow.queueresult();
                try
                {
                    lock (q)
                    {
